...
export default class Getclone {
   ...
   * @param { any } target
   */
  private isObject(target): boolean {
  ...
  private cloneRegister(target) {
    const reFlags: RegExp = new RegExp('\w*$');
  ...
  private cloneFunc(func) {
    const bodyRegister: RegExp = new RegExp('(?<={)(.|\n)+(?=})', 'm');
    const paramRegister: RegExp = new RegExp('(?<=\().+(?=\)\s+{)');
  ...
  private cloneOtherType(target, type) {
    const ctor = target.constructor;
    if (type === boolType || type === numberType || type === stringType || type === errorType || type === dateType) {
